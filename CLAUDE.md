...
- **k-anonymity runs after filtering, before pagination**, and `total` counts only surviving rows. A
  contract declaring `k_anonymity` withholds any group of fewer than `k` rows, refuses the single-study
  route outright, and reports **booleans only** — a withheld-row count would let a caller compute the exact
  number of people matching their filter. Quasi-identifiers must be unconditionally released, and
  k-anonymity alongside a free-text column is refused at load.
...
- **Hermetic (239 tests)** — an `httpx.MockTransport` impersonates the three nodes and serves the real `data/*.json`. No processes or ports; runs in ~5s. This tier is where node failure, timeouts, and total-federation-outage are simulated.
